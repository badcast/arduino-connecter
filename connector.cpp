...
bool device_awake(Connector& con, const command_request* requestCmd, command_responce* responceCmd, uint8_t size_req, uint8_t size_resp, const char* buffer = nullptr, int size = -1)
{
    int alpha;
    ...
    if (requestCmd == nullptr)
        throw std::runtime_error("requestCmd is null");

    if ((requestCmd->commandFlag & 0x1) == SET) {
        if (responceCmd)
            throw std::runtime_error("request method not requirable responce arg");
...
Connector::Connector(const Port& port)
    : __port(port)
{
    if (!connect(__port))
...
{
    if (!isConnected())
        throw std::runtime_error("is not connected");
...
ConnectorUtility::ConnectorUtility(Connector& device)
    : _dev(&device)
{
}
